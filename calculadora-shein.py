...
def main(page: ft.Page):
    ...
    costo_shein = ft.TextField(label="Costo de Producto ($)", value="29000", keyboard_type="number")
    envio_shein = ft.TextField(label="Envío de Producto ($)", value="40000", keyboard_type="number")
    garantia = ft.TextField(label="Garantía de envío ($)", value="1592", keyboard_type="number")
    impuestos = ft.TextField(label="Impuestos Tarjeta/Aduana (%)", value="60", keyboard_type="number")
    ...
    resultado_precio = ft.Text("Precio de Venta Ideal: $0.00", size=20, weight="bold", color="#1dd226") # Verde
    resultado_ganancia = ft.Text("Tu Ganancia Limpia: $0.00", size=18, weight="bold", color="#ea5e0c") # Azul
    ...
    def calcular_rentabilidad(e):
        try:
            ...
            if divisor <= 0:
                ...
                resultado_precio.color = "#e71b17" # Rojo
            ...
            resultado_precio.color = "#00a9fe"
            resultado_ganancia.value = f"Tu Ganancia Limpia: ${ganancia_neta:,.2f}"
            resultado_desglose.value = (
                f"--- Desglose ---\n"
                f"Costo Total Producto (con {impuestos.value}% imp): ${costo_total_shein:,.2f}\n"
                f"Costo Fijo (Producto + Envío ML): ${costo_fijo_total:,.2f}\n"
    ...
    page.add(
        titulo,
        ft.Divider(),
        ft.Text("Costos de Producto", weight="bold", color="#00a9fe"),
        costo_shein, envio_shein, garantia, impuestos,
        
        ft.Divider(),
        ft.Text("Costos y Variables de Venta (Mercado Libre)", weight="bold", color="#00a9fe"),
